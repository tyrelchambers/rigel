...
final class ContextHandoffBuilderTests: XCTestCase {
    func test_podHandoffIncludesNameNamespaceAndDescribe() {
        let pod = Pod(
            ...
            spec: PodSpec(nodeName: "k3s-slave", containers: [Container(name: "postiz", image: "ghcr.io/x/y:latest", resources: nil, ports: nil)]),
        ...
        let prompt = ContextHandoffBuilder.build(.pod(pod, action: .errors, describe: describe, recentEvents: events, logs: nil))
