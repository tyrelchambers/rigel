...
`expose` auto-applies as an **Ingress** (default `expose: "ingress"` with a placeholder host like `example.com`). An Ingress is inert until an ingress controller plus DNS point at it, so unlike a LoadBalancer it costs nothing and never sits `Pending`; the panel shows an "Expose: Ingress" chip (only when the compose actually has published ports, i.e. an Ingress was emitted) plus a host field to set the real domain. **LoadBalancer is NOT auto-applied** — it bills per-hour on cloud and usually sits `Pending` on bare-metal/homelab, so users provision it themselves; it stays reachable as an opt-in one-click when the auto Ingress is undone (the published-port warning returns with a Fix offering LoadBalancer or re-adding Ingress). All fix state feeds the single `convert(compose, { namespace, fixes })` call; the manifests, explainer, and remaining warnings re-render live.
