...
1. **Paste** any manifest into the editor. Multi-document YAML (separated by `---`) is supported, so paste a whole bundle at once.
...
- Same RBAC as the rest of Rigel, so the apply runs as the app's ServiceAccount.
