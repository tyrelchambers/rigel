...
Kubernetes has **no native "move"**, because a resource's namespace is immutable. So **Deployments → right-click → Move to namespace…** picks a target namespace and hands a **clone-then-delete plan to the chat copilot**, which:
...
> ⚠️ **PVC data does not follow.** A recreated PVC binds to a new, empty volume, so the copilot stops and asks before touching storage. Review each step; nothing is bulk-deleted before the new namespace is confirmed working.
