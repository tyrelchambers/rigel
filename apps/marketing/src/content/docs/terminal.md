...
A full `bash` with the cluster tooling baked in: `**kubectl**`, `**helm**`, `**jq**`, plus the `kubectl cnpg` and `kubectl cert-manager` plugins. It inherits the server's environment, so it talks to the **same cluster / kubeconfig / context** as the app, with no extra auth or `--context` juggling.
...
> ⚠️ This is a **full shell with the app's cluster access** (Rigel is a cluster-admin tool). Anyone who can log in can run anything here, so protect Rigel with auth and network controls as usual.
