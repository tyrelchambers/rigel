...
// `@rigel/k8s` package so the panel, the editor, and the shared YAML builders
// all agree on one definition. Mirrors the Swift `ConfigMap` in
// `Sources/Rigel/Cluster/ConfigMap.swift` and `docs/parity/configmaps.md`.
//
...
export type { ConfigMap } from "@rigel/k8s";
