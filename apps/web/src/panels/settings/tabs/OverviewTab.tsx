      ...
      <section className="rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-elevated)] p-5">
        ...
        <div className="mt-4">
          <UpdateBody
            ...
            lastCheckedLabel={
              lastChecked
                ? `Checked ${format(new Date(lastChecked), "p")}`
                : null
            }
          />
        </div>
...
function UpdateBody({
    ...
    return (
      ...
      >
        ...
        <span className="text-[13px] text-[var(--fg-tertiary)]">
          Try again in a moment, or download from the website.
        </span>
      </StatusRow>
    );
  }
    ...
    return (
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          ...
          <span className="text-[13px] text-[var(--fg-tertiary)]">
            Rigel {update.version} has been downloaded.
          </span>
        </StatusRow>
        <PrimaryButton
    ...
    return (
      <div className="flex flex-col gap-2.5 rounded-md border border-[var(--border-subtle)] bg-[var(--surface-sunken)] px-4 py-4">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto font-mono text-xs text-[var(--accent-primary)]">
            {update.progress}%
          </span>
        </div>
  ...
  if (update.status === "available") {
    const highlights = update.releaseNotes
      ? releaseHighlights(update.releaseNotes)
      : [];
    return (
      ...
      <div className="flex flex-1 flex-col gap-0.5">
        ...
        <span className="text-[13px] text-[var(--fg-tertiary)]">
          {update.status === "checking"
            ...
            : version
              ? `Rigel v${version} is the latest version.`
              : "Rigel is the latest version."}
        </span>
      </div>
...
function StatusRow({
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-3.5 rounded-md border border-[var(--border-subtle)] bg-[var(--surface-sunken)] px-4 py-4",
        className,
      )}
    >
      <div
