...
test("renders title, body, the PRO pill, and fires onUpgrade", () => {
  ...
  render(<ProGateCard icon={faCloud} title="Unlock cloud clusters" body="Connect EKS, GKE, AKS." onUpgrade={onUpgrade} />);
...
test("disables the upgrade button when upgradeDisabled", () => {
  render(<ProGateCard icon={faCloud} title="t" body="b" upgradeDisabled />);
  expect(screen.getByRole("button", { name: /upgrade to pro/i })).toBeDisabled();
});

test("renders the optional see-included link", () => {
  render(<ProGateCard icon={faCloud} title="t" body="b" seeIncluded={{ label: "See what's included", href: "https://x" }} />);
