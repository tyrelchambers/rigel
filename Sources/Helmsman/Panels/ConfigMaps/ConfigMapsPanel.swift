    ...
    private var namespaceFilterBar: some View {
        HStack(spacing: 8) {
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 6) {
                    ConfigMapsNamespacePill(label: "all", isActive: viewModel.namespaceFilter == nil) {
                        viewModel.namespaceFilter = nil
                    }
                    ForEach(viewModel.availableNamespaces, id: \.self) { ns in
                        ConfigMapsNamespacePill(label: ns, isActive: viewModel.namespaceFilter == ns) {
...
private struct ConfigMapsNamespacePill: View {
    let label: String
    let isActive: Bool
    let action: () -> Void
