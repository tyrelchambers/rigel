// Bundles the rigel-pr CLI to a single self-contained ESM file (@rigel/k8s
// inlined via the tsconfig paths alias). Shared by `pnpm --filter @rigel/pr-cli
...
await build({
  ...
  outfile: join(root, "dist/rigel-pr.mjs"),
  bundle: true,
  platform: "node",
  format: "esm",
