...
describe("handleMatrixInbound", () => {
  ...
  test("markRead is called with the event id and setTyping true/false for an authorized acted-on message", async () => {
    const raw = syncWith([
    ...
    expect(h.markRead).toHaveBeenCalledWith("$auth1");

    const typingCalls = (h.setTyping as ReturnType<typeof vi.fn>).mock.calls;
    expect(typingCalls[0]).toEqual([true]);
    expect(typingCalls[1]).toEqual([false]);

    // setTyping(true) must happen before the reply and setTyping(false) after.
  ...
  test("markRead and setTyping are NOT called for the bot's own messages", async () => {
    const botId = "@rigel-bot:hs";
    const raw = syncWith([
      { type: "m.room.message", event_id: "$self2", sender: botId, origin_server_ts: 1, content: { msgtype: "m.text", body: "status" } },
    ]);
    const h = fakeHandlers({ sync: vi.fn(async () => raw) });
    await handleMatrixInbound({ ...CTX, allow: [...CTX.allow, botId], botUserId: botId }, h, new SeenEventIds());
    expect(h.markRead).not.toHaveBeenCalled();
